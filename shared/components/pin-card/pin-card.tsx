...
export function PinCard({
  fullImage, title, location, meta, id
}: PinCardProps) {
  return (
    <Link href={`/pin/${id ?? 123}`} className="relative group mb-4 overflow-hidden transition-all duration-200 group">
      <div className="relative w-full rounded-xl overflow-hidden cursor-pointer aspect-auto">
        <img src={fullImage} alt={title} width={400} height={600} className="w-full object-cover transition-transform rounded-xl" />
        <div className="absolute inset-0 bg-black/20 transition-opacity duration-150 group-hover:opacity-100 opacity-0">
          <div className="absolute flex flex-col items-start justify-between p-4 inset-0">
            <div className="flex items-center justify-between w-full mb-2">
              <div className="flex items-center gap-1 px-2 py-1 text-sm text-neutral-900 bg-white/90 rounded-full">
